    ...
    DrawingManager.prototype._bindRectangle = function () {
        ...
        var endAction = function (e) {
            ...
            if (me.type == 'rectangle') {
                var calculate = me._calculate(overlay.overlay, overlay.overlay.getPath());
            }
            else if (me.type == 'circle') {
                var calculate = me._calculate(overlay.overlay, me.point);
            }
            else if (me.type == 'polygon') {
                var calculate = me._calculate(overlay.overlay, (overlay.overlay.getPath()));
                me.overlays.push(overlay.overlay);
                overlay.overlay.disableEditing();
            }
            else if (me.type == 'polyline') {
                var calculate = me._calculate(overlay.overlay, (overlay.overlay.getPath()));
                me.overlays.push(overlay.overlay);
                overlay.overlay.disableEditing();
            }
            me._dispatchOverlayComplete(overlay.overlay, calculate);
            for (var i = 0; i < overlay.overlays.length; i++) {
                if (Array.isArray(overlay.overlays[i])) {
                    for (var k in overlay.overlays[i]) {
                        map.removeOverlay(overlay.overlays[i][k]);
                    }
                } else {
                    map.removeOverlay(overlay.overlays[i]);
                }
            }
            me.close();
